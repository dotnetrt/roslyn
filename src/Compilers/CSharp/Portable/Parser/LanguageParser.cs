...
tryAgain:
        ...
        private void ParseParameterModifiers(SyntaxListBuilder modifiers)
        {
            while (IsParameterModifier(this.CurrentToken.Kind))
            {
                ...
                else if (modifier.Kind == SyntaxKind.ConstKeyword)
                {
                    modifier = CheckFeatureAvailability(modifier, MessageID.IDS_FeatureConstParameters);
                }
...
tryAgain:
        ...
        private static bool IsInvalidSubExpression(SyntaxKind kind)
        {
            switch (kind)
            {
                ...
                case SyntaxKind.CatchKeyword:
                case SyntaxKind.ConstKeyword:       // TODO: const may appear in parameter and argument expressions - verify correctness
                case SyntaxKind.ContinueKeyword:
                case SyntaxKind.DoKeyword:
                case SyntaxKind.FinallyKeyword:
                case SyntaxKind.ForKeyword:
...
tryAgain:
        ...
        private ArgumentSyntax ParseArgumentExpression(bool isIndexer)
        {
            ...
            SyntaxToken refOutOrConstKeyword = null;
            if (this.CurrentToken.Kind == SyntaxKind.RefKeyword || this.CurrentToken.Kind == SyntaxKind.OutKeyword ||
                this.CurrentToken.Kind == SyntaxKind.ConstKeyword)
            {
                refOutOrConstKeyword = this.EatToken();
            }
            ...
            else
            {
                ...
                expression = (refOutOrConstKeyword?.Kind == SyntaxKind.OutKeyword)
            ...
            return _syntaxFactory.Argument(nameColon, refOutOrConstKeyword, expression);
        }
        ...
        private bool ScanExplicitlyTypedLambda(Precedence precedence)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    // Eat 'out' or 'ref' for cases [3, 8]. Even though not allowed in a lambda,
        ...
        private ExpressionSyntax ParseCastOrParenExpressionOrLambdaOrTuple(Precedence precedence)
        {
            ...
            try
            {
                ...
                // Doesn't look like a cast, so parse this as a parenthesized expression or tuple.
                {
                    ...
                    if (this.CurrentToken.Kind == SyntaxKind.CommaToken)
                    {
                        var firstArg = _syntaxFactory.Argument(nameColon: null, refOrOutOrConstKeyword: default(SyntaxToken), expression: expression);
                    ...
                    if (expression.Kind == SyntaxKind.IdentifierName && this.CurrentToken.Kind == SyntaxKind.ColonToken)
                    {
                        ...
                        var firstArg = _syntaxFactory.Argument(nameColon, refOrOutOrConstKeyword: default(SyntaxToken), expression: expression);
                        return ParseTupleExpressionTail(openParen, firstArg);
                    }
            ...
            try
            {
                ...
                while (this.CurrentToken.Kind == SyntaxKind.CommaToken)
                {
                    ...
                    if (expression.Kind == SyntaxKind.IdentifierName && this.CurrentToken.Kind == SyntaxKind.ColonToken)
                    {
                        ...
                        arg = _syntaxFactory.Argument(nameColon, refOrOutOrConstKeyword: default(SyntaxToken), expression: expression);
                    }
                    else
                    {
                        arg = _syntaxFactory.Argument(nameColon: null, refOrOutOrConstKeyword: default(SyntaxToken), expression: expression);
                    }
                ...
                if (list.Count < 2)
                {
                    list.AddSeparator(SyntaxFactory.MissingToken(SyntaxKind.CommaToken));
                    var missing = this.AddError(this.CreateMissingIdentifierName(), ErrorCode.ERR_TupleTooFewElements);
                    list.Add(_syntaxFactory.Argument(nameColon: null, refOrOutOrConstKeyword: default(SyntaxToken), expression: missing));
                }
...
tryAgain:
        ...
        private ParameterSyntax ParseLambdaParameter()
        {
            ...
            SyntaxToken refOrOutOrConstOrParams = this.CurrentToken.Kind == SyntaxKind.RefKeyword || this.CurrentToken.Kind == SyntaxKind.OutKeyword ||
                this.CurrentToken.Kind == SyntaxKind.ParamsKeyword || this.CurrentToken.Kind == SyntaxKind.ConstKeyword
                ? this.EatToken()
                : null;

            TypeSyntax paramType = ShouldParseLambdaParameterType(refOrOutOrConstOrParams)
            ...
            return _syntaxFactory.Parameter(
                default(SyntaxList<AttributeListSyntax>),
                refOrOutOrConstOrParams, paramType, paramName,
                @default: null);
        }

        private bool ShouldParseLambdaParameterType(SyntaxToken refOrOutOrConstOrParams)
        {
            // If we have "ref/out/params" always try to parse out a type.
            if (refOrOutOrConstOrParams != null)
            {
                return true;
            }
